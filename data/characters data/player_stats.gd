...
func add_to_inventory(new_upgrade: Upgrade) -> void:
	if new_upgrade:
		...
		var to_remove = inventory.filter(func(u): return u.name == new_upgrade.name)
		for dup in to_remove:
			remove_from_inventory(dup)
...
func clear_temporary_modifiers():
	var to_remove = temporary_upgrades.duplicate()
...
func purchase_spin(spin_price: int):
	money_dictionary[PlayerStats.MONEY.TOKENS] -= spin_price
