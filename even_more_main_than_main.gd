...
@export var boss_death_delay: float = 3.0

@export_category("Test Scenes")
@export var combat_test_scene: PackedScene
@export var shop_test_scene: PackedScene
@export var systems_test_scene: PackedScene
...
func _ready() -> void:
	...
	# Test scene buttons (created dynamically)
	var grid = cheat_lvl_selection.find_child("GridContainer", true, false)
	if grid:
		if combat_test_scene:
			var combat_btn = Button.new()
			combat_btn.text = "Combat Test"
			combat_btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			combat_btn.size_flags_vertical = Control.SIZE_EXPAND_FILL
			grid.add_child(combat_btn)
			combat_btn.pressed.connect(func(): add_level(combat_test_scene))

		if shop_test_scene:
			var shop_btn = Button.new()
			shop_btn.text = "Shop Test"
			shop_btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			shop_btn.size_flags_vertical = Control.SIZE_EXPAND_FILL
			grid.add_child(shop_btn)
			shop_btn.pressed.connect(func(): add_level(shop_test_scene))

		if systems_test_scene:
			var systems_btn = Button.new()
			systems_btn.text = "Systems Test"
			systems_btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			systems_btn.size_flags_vertical = Control.SIZE_EXPAND_FILL
			grid.add_child(systems_btn)
			systems_btn.pressed.connect(func(): add_level(systems_test_scene))
