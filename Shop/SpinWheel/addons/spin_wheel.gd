...
func place_resources():
	...
	for i in range(remaining_indices.size()):
		...
		if i < tickets_array[PlayerStats.MONEY.TICKETS].size():  # Спочатку tickets_array
			# Вибираємо випадкову нагороду з tickets_array
			var random_reward_index = randi() % tickets_array[PlayerStats.MONEY.TICKETS].size()
			rewards[index]["reward"] = tickets_array[PlayerStats.MONEY.TICKETS][random_reward_index]
			rewards[index]["name"] = "TICKET"
			rewards[index]["slot"].texture = load("res://assets/Store/Ticket.png")
		else:  # Потім tokens_array
			var token_index = i - tickets_array[PlayerStats.MONEY.TICKETS].size()
			var random_reward_index = randi() % tokens_array[PlayerStats.MONEY.TOKENS].size()
			rewards[index]["reward"] = tokens_array[PlayerStats.MONEY.TOKENS][random_reward_index]
