...
func handle_movement(delta: float = 0.016):
	...
	# Нахил моноциклу
	if dir != 0:
		controller.tilt = lerp(controller.tilt, dir * controller.max_tilt, controller.tilt_inertia * delta * 60.0)
	else:
		controller.tilt = lerp(controller.tilt, 0.0, 0.1 * delta * 60.0)
